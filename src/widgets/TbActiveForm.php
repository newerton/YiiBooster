...
class TbActiveForm extends CActiveForm {
    ...
    protected function setDefaultPlaceholder(&$fieldData) {
        if (!is_array($fieldData) || empty($fieldData[0][1]) /* 'textField' */ || !is_array($fieldData[1]) /* ($model, $attribute, $htmlOptions) */
        )
            return;
    ...
    protected function horizontalFieldRow(&$fieldData, &$model, &$attribute, &$rowOptions) {
        ...
        self::addCssClass($rowOptions['labelOptions'], 'col-sm-2 control-label');
